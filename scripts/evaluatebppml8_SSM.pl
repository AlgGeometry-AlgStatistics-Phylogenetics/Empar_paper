...
my $b=$ARGV[1];
    ...
    my $filer='outbppml_SSMGenNonh_b'.$b.'length'.$length;
open(OUTR,">", $filer);


	
system('tar -xf ~/ania_k/Empar_paper/data/simul_balanc8GenNonhSSM/length'.$length.'_b'.$b.'.tar');

	for($i=0;$i < $tests;$i++){
	    
	my $fastafile='length'.$length.'_b'.$b.'_num'.$i.'.fa';

 system('mv '.$fastafile.' align.fa'); 

  system("bppml param=bppsuite8taxa.txt"); 
